...
void ReadPageVerify(volatile unsigned char *memory_address, int flash_address, int pages) {
    int read_byte, zeroth_byte, first_byte, second_byte, counter, error;
    ...
    while (counter < (256 * pages)) {
        ...
        if (*address != read_byte) {
            printf("\r\nError at %X: DRAM: %X    Flash: %X\n", (unsigned int)(uintptr_t)address, *address, read_byte);
            error = 1;
        }
...
void WritePage(volatile unsigned char *memory_address, int flash_address) {
    ...
    volatile unsigned char * address;
    int zeroth_byte, first_byte, second_byte;
    zeroth_byte = (flash_address >> 0) & 0xFF;
    first_byte = (flash_address >> 8) & 0xFF;
    second_byte = (flash_address >> 16) & 0xFF;

    WriteSPIChar(0x06, 0);                                                      // write enable
    WriteSPIChar(0x02, 1);                                                      // page program
    WriteSPIChar(second_byte, 1);                                               // first byte of address 
    WriteSPIChar(first_byte, 1);                                                // second byte of address
    WriteSPIChar(zeroth_byte, 1);                                               // third byte of address
    for (address=memory_address; address<(memory_address + 255); address++) {   // Pages in the flash chip are limited to 256 bytes total; must page program command after 256 bytes are written
        WriteSPIChar(*address, 1);                                              // byte in memory
    }
    WriteSPIChar(*address, 0);                                                  // last byte to be written
...
void WriteProgramToFlash(void) {
    ...
    printf("\r\nErasing flash chip...");
    WriteSPIChar(0x06, 0);                  // write enable
    WriteSPIChar(0xC7, 0);                  // erase chip
