...
{
    WriteProgramToFlash();
    ReadPageVerify((volatile unsigned char *) ProgramStart, 0, 1000);
}
...
{
    unsigned int *RamPtr, counter1=1, increment, count;
    register unsigned int i ;
    unsigned int start, end, j;
    char choice, pattern;
    ...
    unsigned long data_longword;

    printf("\r\nMEMORY TEST\n-----------\n");
    printf("\r\nWould you like to perform the test using bytes, words, or long words?\n");
    printf("\r\n1) Bytes\n2) Words\n3) Long Words\nEnter integer: ");
    choice = _getch();
    ...
    switch (choice) {
        case '1':
            printf("\r\nThe test will be performed using bytes.\n");
            break;
        case '2':
            printf("\r\nThe test will be performed using words.\n");
            break;
        case '3': 
            printf("\r\nThe test will be performed using long words.\n");
            break;
        default:
            printf("\r\nError incurred...");
    }

    printf("\r\nEnter a hexadecimal address to start writing data to memory: ");
    start = Get8HexDigits(0) ;
    printf("\r\nYou entered %x\n", start);
    do {
        if (start < 0x08000000) {
            printf("\r\nAddress input from user to small!\nTry again: ");
        } else if (start >= 0x0BFFFFFF) {
            printf("\r\nAddress input from user to large!\nTry again: ");
        } else {
            address_start = (volatile unsigned char *) start;
            break;
        }
    ...
    if ((choice == '2') || (choice == '3')) {
        if (start % 2 != 0) {
            start = start - 1;
            address_start = (volatile unsigned char *) start;
            printf("\r\nThe start address entered is odd; must be reduced due to hardware limitations when writing words/long words.\nStart address updated: %x", start);
        }
    }

    printf("\r\nEnter a hexadecimal address to stop writing data to memory: ");
    end = Get8HexDigits(0);
    printf("\r\nYou entered %x\n", end);
    do {
        if (end < start) {
            printf("\r\nAddress input from user smaller than start address!\nTry again: ");
        } else if (end > 0x0BFFFFFF) {
            printf("\r\nAddress input from user to large!\nTry again: ");
        } else {
            address_end = (volatile unsigned char *) end;
            break;
        }
    ...
    if ((choice == '2') || (choice == '3')) {
        if (end % 2 != 0) {
            ...
            address_end = (volatile unsigned char *) end;
            printf("\r\nThe end address entered is odd; must be reduced due to hardware limitations when writing words/long words.\nEnd address updated: %x", end);
        }
    }

    printf("\r\nWhat pattern of test data would you like to write to memory?");
    printf("\r\n1) 55...\n2) EE...\n3) 22...\n4) BB...\nEnter integer: ");
    pattern = _getch();
    ...
    if (choice == '1') {
        // Bytes are to be written to memory
        increment = 1;
        ...
        if (pattern == '1') {
            ...
            data_byte = 0x00; // this should never occur 
        }
    }

    if (choice == '2') {
        // Words are to be written to memory
        increment = 2;
        ...
        if (pattern == '1') {
            ...
            data_word = 0x0000; // this should never occur 
        }
    }

    if (choice == '3') {
        // Long words are to be written to memory
        increment = 4;

        if (pattern == '1') {
            data_longword = 0x55555555;
        } else if (pattern == '2') {
            data_longword = 0xEEEEEEEE;
        } else if (pattern == '3') {
            data_longword = 0x22222222;
        } else if (pattern == '4') {
            data_longword = 0xBBBBBBBB;
        } else {
            data_longword = 0x00000000; // this should never occur 
        }
    }

    count = 0;
    for (address=address_start; address<address_end; address=address+increment) {
        if (choice == '1') {
            *((volatile unsigned char *)address) = data_byte;
            if (count == 0) {
                printf("\r\n1 byte written; address %x....", address);
            } else {
                if (count % 1000 == 0) {
                    printf("\r\n%i bytes written; address %x....", count + 1, address);
                }
            }
        } 
        if (choice == '2') {
            *((volatile unsigned short *)address) = data_word;
            if (count == 0) {
                printf("\r\n1 word written; address %x....", address);
            } else {
                if (count % 1000 == 0) {
                    printf("\r\n%i words written; address %x....", count + 1, address);
                }
            }
        }
        if (choice == '3') {
            *((volatile unsigned long *)address) = data_longword;
            if (count == 0) {
                printf("\r\n1 long word written; address %x....", address);
            } else {
                if (count % 1000 == 0) {
                    printf("\r\n%i long words written; address %x....", count + 1, address);
                }
            }
        }
    ...
    if (choice == '1') {
        printf("\r\n\n%i bytes written.", count);
    }
    if (choice == '2') {
        printf("\r\n\n%i words written", count);
    }
    if (choice == '3') {
        printf("\r\n\n%i long words written", count);
    }
...
void main(void)
{
    ...
    char *Name = "Alex Manak";
    char *StudentNumber = "49529324";
