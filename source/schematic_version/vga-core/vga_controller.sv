...
module vga_controller(	vga_clock, resetn, pixel_colour, memory_address,
	...
	/* Set this parameter to "TRUE" if you only wish to use grayscale colours. Doing so will reduce
	...
	input [((MONOCHROME == "TRUE") ? (BITS_PER_COLOUR_CHANNEL-1) : (BITS_PER_COLOUR_CHANNEL*3-1)):0] pixel_colour;
	...
	begin
		...
		begin
			for (index = 0; index < BITS_PER_COLOUR_CHANNEL; index = index + 1)
			begin
				VGA_R[index+10-BITS_PER_COLOUR_CHANNEL] <= on_screen & pixel_colour[index];
				VGA_G[index+10-BITS_PER_COLOUR_CHANNEL] <= on_screen & pixel_colour[index];
				VGA_B[index+10-BITS_PER_COLOUR_CHANNEL] <= on_screen & pixel_colour[index];
			end
		end
	end
